...
export class FirebaseService {
    ...
    private _allItems: Array<Speaker> = [];
    ...
    add(speakerData: string) {  
        ...
        return firebase.push(
                "/Speakers",
                {
                    ...
                    "approved": speakerParse.approved,
                    "UID": BackendService.token
                }
            ).then(
                function (result:any) {
                BackendService.speaker = speakerData;
                return 'Registered!';
            },
            function (errorMessage:any) {
    ...
    // getSpeaker(uid: string) {
    //     return firebase.database().ref('/Speakers/' + uid).once('value').then(function(snapshot) {
    ...
    //     //   BackendService.apiUrl + "Speakers" + uid,
    //     //   { headers: headers }
    //     // )
...
//   }
  ...
  //   return new Observable((observer: any) => {
  //     observer.next(this._allItems.filter(s => s.UID === id)[0]);
  //   }).share();
  // }
